...
- Lertaro 的 Hook 进程会在前台窗口每次变化时轮询一次活动路径，所以这次临时读取必须完全不可见：查询期间暂停命令行控件的重绘，读到的路径在清空后还会复核一次并重试，避免 Double Commander 稍后写入的文本留在输入框里。读取本身在 150ms 内轮询，避免把偶发的空读当成“没有路径”而清空已有搜索范围。
